function startApp() {
    ...
    const appId = 'kid_rJLVHwPMl';
    const appPass = 'd90fdf48560f435182c036ff395682f1';
    ...
    function processCreateAd(formId) {
        ...
        let image = Number($(formId).find('input[name="image"]').val().trim());
    ...
    function renderAds(ads) {
        ...
        function appendAdminCell(row, ad) {
            let displayLink = $('<a href="#">[Read More]</a>').on('click', () => displayAdvert(ad));
            let deleteLink = $('<a href="#">[Delete]</a>').on('click', () => deleteAd(ad._id));
            let editLink = $('<a href="#">[Edit]</a>').on('click', () => editAd(ad));
            if (ad._acl.creator === sessionStorage.getItem('userId')) {
                row.append($('<td>').append(displayLink, ' ', deleteLink, ' ', editLink));
            }
            ...
            function displayAdvert(ad) {
                ...
                $.ajax({
                    ...
                    .then(displayAdSuccess)
                    .catch(errorHandler);

                function displayAdSuccess(ad) {
                    let html = $('<div>');
                    html.append(
