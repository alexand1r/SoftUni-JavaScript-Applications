...
function doStuff() {
    let BetsTable = React.createClass({
        render: function () {
            ...
            elements = elements.map(function (element, ind) {
                return <tr key={ind}>
    ...
    let MatchesTable = React.createClass({
        placeBet: function (ev) {
            ...
            this.setState({
                elements: this.transformer(this.props.elements)
            })
        },
        refresh: function () {
            this.render();
    ...
    let ButtonControls = React.createClass({
        showMyBets: function () {
            ReactDOM.render(
            <BetsTable elements={myBets}/>,
                $('.content-holder')[0]
            );
        },
        ...
        render: function () {
            ...
            return <div className="button-holder">
                <button id="bets" className="button" onClick={that.showMyBets}>My Bets</button>
            <button id="matches" className="button" onClick={that.showMatches}>Matches</button>
            </div>
        }
    });
    ...
    return {AppView, ButtonControls, MatchesTable, BetsTable}
}
...
function init() {
    let classes = doStuff();
    ReactDOM.render(
        React.createElement(classes.AppView),
